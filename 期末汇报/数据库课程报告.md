...
- 吴宇涵：基于角色的访问控制、登陆认证模块；教师管理模块；UI的实现
- 翁子昂：数据库结构设计；校建管理模块和学生管理模块；UI的实现
- 孙睿江：学生选课模块和成绩管理模块；UI设计和实现

实验流程包括：

数据库结构设计、功能设计、后端系统实现、安全性分析、前端UI设计和实现、测试和修改。
...
​	总之，我们较完整地实现了具有基本功能以及登录验证、权限控制、防止CRSF攻击和SQL注入攻击等特性的基于角色权限安全的教务信息管理系统，对其中的信息系统安全机制有了更多的理解。
