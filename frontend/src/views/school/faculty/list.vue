  ...
  <div class="app-container">
    <el-form :model="queryData" ref="queryForm" :inline="true">
      ...
      <el-form-item>
        ...
        <router-link :to="{path:'/school/faculty/edit'}" class="link-left">
    ...
    <el-table v-loading="listLoading" :data="tableData" border fit highlight-current-row style="width: 100%">
      ...
      <el-table-column width="270px" label="操作" align="center">
        <template slot-scope="{row}">
          <router-link :to="{path:'/school/faculty/edit', query:{facultyCode:row.facultyCode}}" class="link-left">
            <el-button size="mini" >编辑</el-button>
          </router-link>
          <el-button  size="mini" type="danger" @click="deleteFaculty(row)" class="link-left">删除</el-button>
        </template>
      </el-table-column>
    </el-table>
  ...
  methods: {
    ...
    deleteFaculty (row) {
      ...
      this.$confirm('确定删除 ?', '提示', {
        ...
        API.deleteFaculty(row.facultyCode).then(re => {
          if (re.code === '0') {
            _this.search()
