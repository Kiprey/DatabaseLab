  ...
  <div class="app-container">
    <el-form :model="queryData" ref="queryForm" :inline="true">
      ...
      <el-form-item>
        ...
        <router-link :to="{path:'/education/courseClass/edit'}" class="link-left">
    ...
    <el-table v-loading="listLoading" :data="tableData" border fit highlight-current-row style="width: 100%">
      ...
      <el-table-column width="270px" label="操作" align="center">
        <template slot-scope="{row}">
          <router-link :to="{path:'/education/courseClass/edit', query:{courseClassID:row.courseClassID}}" class="link-left">
            <el-button size="mini" >编辑</el-button>
          </router-link>
          <el-button  size="mini" type="danger" @click="deleteCourseClass(row)" class="link-left">删除</el-button>
        </template>
      </el-table-column>
    </el-table>
...
export default {
  ...
  methods: {
    search () {
      ...
      API.queryCourseClass(this.queryData, queryParam).then(data => {
        let _this = this
        if (data.code === '0') {
          const re = data.data
